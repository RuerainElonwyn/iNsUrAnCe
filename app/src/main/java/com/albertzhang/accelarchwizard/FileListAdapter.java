...
/**
 * Created by Albert Zhang on 17/4/2016.
 */
public class FileListAdapter extends RecyclerView.Adapter<FileListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(FileListAdapter.ViewHolder holder, int position) {
        final File file = AccelerationTimeGraph.fileList.get(position);
    ...
    public int getItemCount() {
        return AccelerationTimeGraph.fileList.size();
    }
                    ...
                    popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            if(item.equals(popupMenu.getMenu().getItem(0))){
                                File file = new File(filePath + File.separator + AccelerationTimeGraph.fileList.get(position).getName());
                                if(file.exists()){
                                    if(file.delete()){
                                        AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                        builder.setIcon(R.drawable.acceleration_vector_white);
                                        builder.setTitle("File deleted");
                                        builder.setMessage("File deleted successfully: " + AccelerationTimeGraph.fileList.get(position));
                                        builder.show();
                                        AccelerationTimeGraph.fileList.remove(position);
                                        notifyItemRemoved(position);
                                    }else{
                                        AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                        builder.setIcon(R.drawable.acceleration_vector_white);
                                        builder.setTitle("Error");
                                        builder.setMessage("File failed to delete: "+AccelerationTimeGraph.fileList.get(position));
                                        builder.show();
                                    }
                                }else{
                                    AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                    builder.setIcon(R.drawable.acceleration_vector_white);
                                    builder.setTitle("Error");
                                    builder.setMessage("The file you requested may have been deleted or corrupted: "+AccelerationTimeGraph.fileList.get(position));
                                    builder.show();
                                }
                            }
                            return true;
