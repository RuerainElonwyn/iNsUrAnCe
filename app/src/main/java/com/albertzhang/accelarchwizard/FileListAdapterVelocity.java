 ...
 */
public class FileListAdapterVelocity extends RecyclerView.Adapter<FileListAdapterVelocity.ViewHolder> {
    ...
    public FileListAdapterVelocity(Context context){
        this.context = context;
    }

    @Override
    public FileListAdapterVelocity.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(FileListAdapterVelocity.ViewHolder holder, int position) {
        final File file = VelocityTimeGraph.fileList.get(position);
    ...
    public int getItemCount() {
        return VelocityTimeGraph.fileList.size();
    }
                    ...
                    popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            if(item.equals(popupMenu.getMenu().getItem(0))){
                                File file = new File(filePath + File.separator + VelocityTimeGraph.fileList.get(position).getName());
                                if(file.exists()){
                                    if(file.delete()){
                                        AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                        builder.setIcon(R.drawable.acceleration_vector_white);
                                        builder.setTitle("File deleted");
                                        builder.setMessage("File deleted successfully: " + VelocityTimeGraph.fileList.get(position));
                                        builder.show();
                                        VelocityTimeGraph.fileList.remove(position);
                                        notifyItemRemoved(position);
                                    }else{
                                        AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                        builder.setIcon(R.drawable.acceleration_vector_white);
                                        builder.setTitle("Error");
                                        builder.setMessage("File failed to delete: "+VelocityTimeGraph.fileList.get(position));
                                        builder.show();
                                    }
                                }else{
                                    AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                                    builder.setIcon(R.drawable.acceleration_vector_white);
                                    builder.setTitle("Error");
                                    builder.setMessage("The file you requested may have been deleted or corrupted: "+VelocityTimeGraph.fileList.get(position));
                                    builder.show();
                                }
                            }
                            return true;
